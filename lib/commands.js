...
class Commands {
    /**
     * Resolves file paths, expanding any glob patterns
     *
     * @param {string[]} filePaths - file paths which may contain glob patterns
     * @returns {string[]} - resolved file paths with expanded glob patterns
     * @private
     */
    static resolveFilePaths(filePaths) {
        const resolvedFileNames = [];

        // Process each file path, expanding any glob patterns
        for (const filePath of filePaths) {
            const normalizedPath = path.normalize(filePath);
            if (normalizedPath.includes('*') || normalizedPath.includes('?') || normalizedPath.includes('{')) {
            // Path contains glob pattern, resolve it
                const matches = glob.sync(normalizedPath, { windowsPathsNoEscape: true });
                for (const match of matches) {
                    const normalizedMatch = path.normalize(match);
                    // Add only unique entries to the array
                    if (!resolvedFileNames.includes(normalizedMatch)) {
                        resolvedFileNames.push(normalizedMatch);
                    }
                }
            } else {
            // Regular path, add if not already included
                if (!resolvedFileNames.includes(filePath)) {
                    resolvedFileNames.push(filePath);
                }
            }
        }
    ...
    static async validate(sample, ctoFiles, options) {
        ...
        const resolvedCtoFiles = Commands.resolveFilePaths(ctoFiles);
        const modelManager = await ModelLoader.loadModelManager(resolvedCtoFiles, options);
    ...
    static async compile(target, ctoFiles, output, options) {
        ...
        const resolvedCtoFileNames = Commands.resolveFilePaths(ctoFiles);

        const modelManager = await ModelLoader.loadModelManager(resolvedCtoFileNames, modelManagerOptions);
    ...
    static async get(ctoFiles, output) {
        const resolvedCtoFiles = Commands.resolveFilePaths(ctoFiles);
        const modelManager = await ModelLoader.loadModelManager(resolvedCtoFiles);
    ...
    static async generate(ctoFiles, concept, mode, options) {
        ...
        const resolvedCtoFiles = Commands.resolveFilePaths(ctoFiles);
        const modelManager = await ModelLoader.loadModelManager(resolvedCtoFiles, modelManagerOptions);
        if (options && options.metamodel) {
            modelManager.addCTOModel(MetaModelUtil.metaModelCto);
        }
        const factory = new Factory(modelManager);
    ...
    /**
     * Lints Concerto models
     *
     * @param {string} modelFile - Path to Concerto model file to lint
     * @param {string} ruleset - Path to a custom Spectral ruleset or 'default' to use the built-in ruleset
     * @param {string[]} excludeNamespaces - One or more namespaces to exclude from linting results
     * @returns {Promise<object[]>} - The linting results
     */
    static async lintModel(modelFile, ruleset, excludeNamespaces) {
        try {
            ...
            const lintOptions = {
                ruleset: ruleset,
                excludeNamespaces: excludeNamespaces || ['concerto.*', 'org.accord.*']
            };

            const results = await lintModel(content, lintOptions);

            return results;
        } catch (e) {
            throw new Error(`Error linting model: ${e.message}`);
        }
    }
}
